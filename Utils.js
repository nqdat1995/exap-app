...
export const GetPetInfo = async (url) => {
  let data = {
    abilities: [
      {
        ability: {
...
export const DownloadImage = async (url, name) => {
  try {
    let location = FileSystem.documentDirectory + `${name}.png`;
    await FileSystem.downloadAsync(url, location);
    alert("Saved!");
  } catch (error) {
    alert(error);
  }
...
export const BuildCollapseFromObject = (data) => {
  let collapses = [];
  ...
  for (let i = 0; i < keys.length; i++) {
    collapses.push(
      <Collapse
        key={keys[i]}
        title={keys[i]}
        content={JSON.stringify(data[keys[i]])}
      />
    );
  }
  return collapses;
...
export const BuildCollapseFromArray = (data = []) => {
  let collapses = [];
